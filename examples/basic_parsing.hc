...
// Predicate for filtering — named function avoids match-inside-lambda codegen issue
fun age_under_30(row: list<string>) : bool {
  match str_at(row, 1) {
    Some(age_str) => to_int(age_str) < 30,
    None => false
  }
}

fun print_name_from_map(m: list<(string, string)>) {
...
fun main() {
  ...
  // Filter rows using a named predicate function
  let young = csv_filter_rows(table, age_under_30)
  println("")
  println("Under 30:")
